...
async def update_order(
    ...
    if invalid_fields:
        ...
        return {
            ...
            "data": None,
            "error": (
                f"Fields not patchable: {', '.join(sorted(invalid_fields))}. "
                f"Allowed: {allowed}. Use InternalInfo for comments."
            ),
...
async def send_order(
    ...
    if transport_type not in valid_transport_types:
        return {
            ...
            "error": (
                ...
                f"Valid types: {', '.join(sorted(valid_transport_types))}. Use 'SMTP' for email."
            ),
    ...
    return await client.request("POST", "/orders/commands/send", **kwargs)
